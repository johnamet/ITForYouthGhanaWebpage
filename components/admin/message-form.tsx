...
export function MessageForm({ message }: { message: CmsContactMessage }) {
  ...
  const [values, setValues] = useState({ status: message.status, notes: "" });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setNotice({ type: "success", message: payload.message || "Message updated." });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <select
            value={values.status}
            onChange={(e) => setValues((v) => ({ ...v, status: e.target.value as CmsContactMessage["status"] }))}
        ...
        <div>
          <label className="text-sm font-bold text-brand-ink">From</label>
          <input disabled value={`${message.name} <${message.email}>`} className={inputClassName} />
        </div>
      </div>

      <div>
        <label className="text-sm font-bold text-brand-ink">Message</label>
        <textarea disabled value={message.message} className={textareaClassName} />
      </div>

      <div>
        <label className="text-sm font-bold text-brand-ink">Internal notes</label>
        <textarea
          value={values.notes}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button type="submit" disabled={isSubmitting} className="inline-flex items-center gap-2 rounded-full bg-brand-gold px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-md disabled:opacity-60">
