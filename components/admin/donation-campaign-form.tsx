...
const numberInput =
  "mt-2 w-40 rounded-2xl border border-brand-border bg-white px-3 py-2 text-sm text-brand-ink outline-none transition focus:border-brand-gold focus:ring-2 focus:ring-brand-gold/20";
...
export function DonationCampaignForm({ initial }: Props) {
  ...
  const update = (key: keyof DonationCampaignContent, value: unknown) => {
    setValues((v) => ({ ...v, [key]: value as any }));
  };

  const updateArray = (key: keyof DonationCampaignContent, csv: string) => {
    const arr = csv
      .split("\n")
      .map((s) => s.trim())
      .filter(Boolean);
    setValues((v) => ({ ...v, [key]: arr as any }));
  };
      ...
      <section className="rounded-[30px] border border-brand-border bg-white p-6 shadow-sm lg:p-8">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <input className={input} value={values.secondaryCta?.label ?? ""} onChange={(e) => update("secondaryCta", { ...values.secondaryCta, label: e.target.value })} />
          </div>
          <div>
            <label className="text-sm font-bold text-brand-ink">Secondary CTA href</label>
            <input className={input} value={values.secondaryCta?.href ?? ""} onChange={(e) => update("secondaryCta", { ...values.secondaryCta, href: e.target.value })} />
          </div>
        </div>
      </section>
