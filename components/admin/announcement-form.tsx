...
export function AnnouncementForm({ initial }: AnnouncementFormProps) {
  ...
  const update = (key: keyof Announcement, value: unknown) => {
    setValues((v) => ({ ...v, [key]: value as any }));
  };

  const updateCta = (key: "label" | "href", value: string) => {
    setValues((v) => ({ ...v, cta: { ...(v.cta ?? {}), [key]: value } } as Announcement));
  };
      ...
      <section className="rounded-[30px] border border-brand-border bg-white p-6 shadow-sm lg:p-8">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <select id="variant" value={values.variant} onChange={(e) => update("variant", e.target.value)} className={inputClass}>
