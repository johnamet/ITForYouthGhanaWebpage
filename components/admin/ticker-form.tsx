...
export function TickerForm({ initial }: Props) {
  ...
  const update = (key: keyof MarqueeTickerContent, value: unknown) => setValues((v) => ({ ...v, [key]: value as any }));
  ...
  const updateItem = (i: number, key: keyof MarqueeTickerItem, val: string) =>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div>
            ...
            <select className={smallInput} value={values.speed ?? "medium"} onChange={(e) => update("speed", e.target.value)}>
            ...
            <div key={`${it.label}-${i}`} className="grid items-end gap-4 rounded-2xl border border-brand-border p-4 md:grid-cols-[1fr_1fr_1fr_auto_auto]">
              ...
              <div>
                ...
                <select
                  ...
                  value={it.type ?? ""}
                  onChange={(e) => updateItem(i, "type" as keyof MarqueeTickerItem, (e.target.value || "") as any)}
                >
