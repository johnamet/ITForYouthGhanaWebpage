...
export function HeroSlidesForm({ initial }: Props) {
  ...
  const update = (index: number, key: keyof HeroSlide, value: unknown) => {
    setSlides((arr) => {
      const next = [...arr];
  ...
  const updateCta = (index: number, which: "primary" | "secondary", key: "label" | "href", value: string) => {
    setSlides((arr) => {
      ...
      if (which === "primary") {
        slide.cta.primary = { ...slide.cta.primary, [key]: value } as any;
      } else {
        slide.cta.secondary = { ...(slide.cta.secondary ?? { label: "", href: "" }), [key]: value } as any;
      }
      next[index] = slide;
      return next;
    });
