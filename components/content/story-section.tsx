    ...
    <section className={`${toneClasses[tone]} py-section-lg`}>
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:gap-20 lg:px-8">
        ...
        <div className="max-w-xl">
          {eyebrow ? <p className={`text-sm font-bold uppercase tracking-[0.18em] ${isDark ? "text-brand-warm" : "text-brand-gold"}`}>{eyebrow}</p> : null}
          <h2 className={`mt-4 font-heading text-4xl font-bold sm:text-5xl lg:text-6xl ${isDark ? "text-white" : "text-brand-ink"}`}>{title}</h2>
          {headline ? <p className={`mt-5 text-xl font-semibold leading-snug sm:text-2xl ${isDark ? "text-white/90" : "text-brand-navy"}`}>{headline}</p> : null}
          <p className={`mt-6 text-lg leading-8 ${isDark ? "text-white/80" : "text-slate-700"}`}>{description}</p>
          {supportingText ? <p className={`mt-5 border-l-2 pl-5 text-base leading-7 ${isDark ? "border-brand-warm text-white/75" : "border-brand-gold text-slate-600"}`}>{supportingText}</p> : null}
