...
export function PageHeader({
  ...
  return (
    <header className={cn("space-y-3", centered && "mx-auto text-center", className)}>
      {eyebrow?.trim() ? <p className="text-[0.65rem] font-bold uppercase tracking-[0.28em] text-brand-gold">{eyebrow}</p> : null}
      ...
      {description?.trim() ? <p className={cn("text-[0.95rem] leading-[1.8] text-slate-500", centered ? "mx-auto max-w-2xl" : "max-w-2xl")}>{description}</p> : null}
    </header>
  );
}
