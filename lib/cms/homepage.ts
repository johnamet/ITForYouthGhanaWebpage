...
export async function getCmsAnnouncement(): Promise<Announcement> {
  ...
  try {
    ...
    return {
      ...
      cta:
        raw && typeof raw === "object" && raw.cta && typeof (raw as any).cta === "object"
          ? {
              label: String((raw as any).cta.label ?? seedAnnouncement.cta?.label ?? "Learn more"),
              href: String((raw as any).cta.href ?? seedAnnouncement.cta?.href ?? "/"),
            }
...
export async function getCmsHeroSlides(): Promise<HeroSlide[]> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const slides = getDocField<unknown[]>(doc.data() as any, "heroSlides") ?? [];
...
export async function getCmsHomepageTicker(): Promise<MarqueeTickerContent> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const ticker = getDocField<MarqueeTickerContent>(doc.data() as any, "ticker");
...
export async function getCmsProgrammeShowcase(): Promise<ProgrammeShowcaseItem[]> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const items = getDocField<unknown[]>(doc.data() as any, "programmeShowcase") ?? [];
...
export async function getCmsDonationCampaign(): Promise<DonationCampaignContent> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const campaign = getDocField<DonationCampaignContent>(doc.data() as any, "donationCampaign");
...
export async function getCmsFeaturedStory(): Promise<FeaturedStoryContent> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const story = getDocField<FeaturedStoryContent>(doc.data() as any, "featuredStory");
...
export async function getCmsJoinCtaCards(): Promise<JoinCtaCard[]> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const cards = getDocField<unknown[]>(doc.data() as any, "joinCtaCards") ?? [];
...
export async function getCmsNewsletterSignup(): Promise<NewsletterSignupContent> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const content = getDocField<NewsletterSignupContent>(doc.data() as any, "newsletterSignup");
...
export async function getCmsFloatingElements(): Promise<FloatingElementsContent> {
  ...
  try {
    const doc = await db.collection(FIREBASE_COLLECTIONS.homepage).doc(DOC_ID).get();
    const content = getDocField<FloatingElementsContent>(doc.data() as any, "floatingElements");
