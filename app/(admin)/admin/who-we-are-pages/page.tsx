  ...
  },
  {
    ...
    render: (page) => (
      <div className="flex flex-wrap gap-2">
        ...
        <Link
          href={`/admin/who-we-are-pages/${page.slug}`}
...
export default async function AdminWhoWeAreDynamicPagesPage() {
  const pages = await getCmsWhoWeAreDynamicPages(true);
  ...
  const metrics: AdminMetric[] = [
    {
      ...
      description: "Dynamic pages created below the Who We Are hub.",
      status: "active",
    },
    {
      label: "Published",
      value: String(publishedCount),
      description: "Visible at /who-we-are/{slug}.",
      status: "published",
    },
    {
      label: "Drafts",
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        eyebrow="Who We Are"
        title="Custom pages"
        description="Create extra public pages such as Board of Directors, Governance, Advisors, or any other organisation profile page."
        icon={<FilePlus2 className="h-5 w-5" />}
        primaryAction={{ label: "Create page", href: "/admin/who-we-are-pages/new" }}
      />
      ...
      <AdminDataTable columns={columns} rows={pages} emptyMessage="No custom Who We Are pages yet." />
    </div>
  );
}
